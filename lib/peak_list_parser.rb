...
module Bruker
  module XML
    ...
    class PLParser
      ...
      def parseDetails(ds)
        c = ds.content
        f1 = /F1=(.*)ppm, F2/.match(c)[1].to_f
        f2 = /F2=(.*)ppm, MI/.match(c)[1].to_f
        mi = /MI=(.*)cm, MAXI/.match(c)[1].to_f
        mx = /MAXI=(.*)cm, PC/.match(c)[1].to_f
        pc = /PC=(.*)/.match(c)[1].to_f
      ...
      def parsePL(xmlDoc)
        ...
        @doc.css("PeakList1D").each do |l_node|
          ...
          ps.map!{|p| Peak1D.new(p.at_css("@F1").value.to_f, p.at_css("@intensity").value.to_f, p.at_css("@type").value.to_i)}
